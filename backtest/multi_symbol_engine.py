...
class MultiSymbolBacktestEngine:
    def run(self, data: Dict[str, pd.DataFrame], mode: str = "walkforward") -> dict:
        ...
        logger.info(f"[MultiBacktest] symbols={list(data.keys())} mode={mode}")
        featured = {}
        ...
        if mode == "walkforward":
            return self._walk_forward(featured)
        return self._simple_split(featured)
    ...
    def _align_dataframes(self, featured: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        common_sorted = sorted(common_index)
        return {sym: df.loc[common_sorted] for sym, df in featured.items()}
    ...
    def _walk_forward(self, featured: Dict[str, pd.DataFrame], train_bars: int = 700, test_bars: int = 200, step_bars: int = 100) -> dict:
        ...
        result["mode"] = "multi-symbol-walkforward"
        result["symbols_traded"] = self._symbol_breakdown(all_trades)
    ...
    def _simple_split(self, featured: Dict[str, pd.DataFrame], train_ratio: float = 0.7) -> dict:
        ...
        test_data = {sym: df.iloc[split:] for sym, df in aligned.items()}
        trades, _ = self._simulate_multi(test_data, start_bar=split)
        if not trades:
            return self._no_trade_error(aligned)
        result = self._compute_metrics(trades)
        result["mode"] = "multi-symbol-simple"
    ...
    def _simulate_multi(self, data: Dict[str, pd.DataFrame], start_bar: int = 0):
        ...
        for i in range(n_bars):
            ...
            if in_trade and trade_symbol in dfs:
                ...
                high = float(row.get("high", row["close"])); low = float(row.get("low", row["close"])); close = float(row["close"])
                ...
                if not tp1_done:
                    ...
                    if hit_tp1:
                        realize(tp1, min(tp1_pct, remaining), "TP1", i, trade_symbol)
                ...
                if remaining > 0 and not tp2_done:
                    ...
                    if hit_tp2:
                        realize(tp2, min(tp2_pct, remaining), "TP2", i, trade_symbol)
                        tp2_done = True
                ...
                if hit_sl or expired:
                    realize(sl if hit_sl else close, remaining, "TRAIL" if hit_sl else "TIME", i, trade_symbol)
                    in_trade = False
                    if capital <= 0:
                        break
                elif remaining <= 1e-9:
            ...
            if best_signal is None or best_symbol is None:
                continue
            row = dfs[best_symbol].iloc[i]
            close = float(row["close"])
            atr_norm = max(float(row.get("atr_norm", 0.002)), float(getattr(cfg, "MIN_ATR_NORM", 0.001)))
            trade_side = int(best_signal["direction"])
            entry_score_val = float(best_signal.get("fusion_score", 0))
            entry_capital = capital
            entry_px = close * (1 + trade_side * SLIPPAGE)
            atr_abs = max(entry_px * atr_norm, entry_px * 0.001)
            lookback = int(getattr(cfg, "CHANDELIER_LOOKBACK", 22))
            start_idx = max(0, i - lookback + 1)
            hh = float(dfs[best_symbol]["high"].iloc[start_idx:i + 1].max())
            ll = float(dfs[best_symbol]["low"].iloc[start_idx:i + 1].min())
            sl = (hh - atr_abs * sl_mult) if trade_side == 1 else (ll + atr_abs * sl_mult)
            sl = min(sl, entry_px - atr_abs * 0.5) if trade_side == 1 else max(sl, entry_px + atr_abs * 0.5)
            tp1 = entry_px + trade_side * atr_abs * tp1_mult
            tp2 = entry_px + trade_side * atr_abs * tp2_mult
            entry_bar = i; trade_symbol = best_symbol
            peak = float(row.get("high", close)); trough = float(row.get("low", close))
            remaining = 1.0; tp1_done = tp2_done = False; in_trade = True; trades_today += 1
    ...
    def _compute_signal(self, row: pd.Series) -> dict:
        ...
        if vol_z > float(getattr(cfg, "MAX_VOL_ZSCORE", 3.5)):
            return {"trade": False, "reason": "vol_spike"}
        if atr_norm < float(getattr(cfg, "MIN_ATR_NORM", 0.001)):
            return {"trade": False, "reason": "dead_vol"}
        scores = []
        ema_stack = float(row.get("ema_stack", 0)); scores.append(ema_stack * 1.2)
        vwap_dist = float(row.get("dist_vwap", 0)); vwap_sig = 1 if vwap_dist > 0.0004 else (-1 if vwap_dist < -0.0004 else 0); scores.append(vwap_sig * 0.9)
        ...
        stoch_cross = float(row.get("stoch_cross", 0)); scores.append(stoch_cross * 0.6)
        ...
        market_structure = float(row.get("market_structure", 0)); scores.append(market_structure * 0.7)
        macd_signal = float(row.get("macd_signal", 0)); scores.append(macd_signal * 0.5)
        bb_pos = float(row.get("bb_position", 0.5)); bb_sig = 1 if bb_pos < 0.25 else (-1 if bb_pos > 0.75 else 0); scores.append(bb_sig * 0.5)
        ...
        rsi_tilt = 1 if rsi_val > 52 else (-1 if rsi_val < 48 else 0)
        ...
        direction = 1 if fusion_score > 0 else -1; abs_score = abs(fusion_score) + vol_boost
        regime_block = float(getattr(cfg, "REGIME_BLOCK_THRESHOLD", 0.25))
        if regime_bias == 1 and direction == -1 and abs(entry_score) < regime_block:
        ...
        adx = float(row.get("adx_trend_strength", 99) or 99)
        if adx < float(getattr(cfg, "MIN_ADX", 18)):
            return {"trade": False, "reason": "adx_filter", "fusion_score": fusion_score, "abs_score": abs_score}
        if abs_score < float(getattr(cfg, "FUSION_THRESHOLD", 0.18)):
    ...
    def _symbol_breakdown(self, trades: list) -> dict:
        ...
        for t in trades:
            ...
            breakdown[sym]["trades"] += 1; breakdown[sym]["pnl"] += t["pnl"]
            if t["pnl"] > 0:
                breakdown[sym]["wins"] += 1
        for sym in breakdown:
            n = breakdown[sym]["trades"]
            breakdown[sym]["win_rate"] = round(breakdown[sym]["wins"] / n, 3) if n > 0 else 0
    ...
    def _no_trade_error(self, featured: dict) -> dict:
        ...
        for _, df in featured.items():
            for _, row in df.tail(100).iterrows():
                r = self._compute_signal(row).get("reason", "unknown")
                reasons[r] = reasons.get(r, 0) + 1
        return {"error": f"No trades generated across {list(featured.keys())}", "threshold": float(getattr(cfg, "FUSION_THRESHOLD", 0.18)), "block_reasons": reasons}

    def _compute_metrics(self, trades: list) -> dict:
        if not trades:
            return {"error": "No trades"}
        df_t = pd.DataFrame(trades); wins = df_t[df_t["pnl"] > 0]; losses = df_t[df_t["pnl"] <= 0]
        win_rate = len(wins) / len(df_t); avg_win = float(wins["pnl"].mean()) if len(wins) else 0.0; avg_loss = float(losses["pnl"].mean()) if len(losses) else 0.0
        ...
        final_cap = float(df_t["capital"].iloc[-1]); total_return = (final_cap - initial) / initial
        rets = df_t["pnl_pct"].values / 100; sharpe = float(rets.mean() / (rets.std() + 1e-9)) * np.sqrt(252) if len(rets) > 1 else 0.0
    ...
    def _equity_curve(self, trades: list) -> list:
        ...
        for t in trades:
            curve.append({"bar": t["bar"], "capital": t["capital"], "symbol": t.get("symbol", "")})
        return curve
